...
/**
 * 传导限伤引擎 — 统一限伤核心（2026-08-07 重构终态，参考 flashfur HealthManager 思路）。
 *
 ...
 *       <b>永不豁免</b>（无"已限过"标记）——寰宇支配之剑等模组在一次攻击里先 hurt() 再直接 setHealth(0)，
...
public final class ConductionDamageLimiter {
    ...
    public static float limitHurt(LivingEntity target, DamageSource source, float amount, long gameTick) {
        ...
        double cap = readAttr(target, YizAttributes.CONDUCTION_CAP);
        // 保底限伤（flashfur 式，写死不依赖属性）：血量闭包实体即使属性未挂/为 0 也默认限 25% 上限，
        // 防止外部模组（如寰宇支配之剑 setHealth(0)/巨量伤害）因属性挂载失败而穿透秒杀。
    ...
    /** 读属性值（null 安全）。 */
    private static double readAttr(LivingEntity entity, net.minecraft.core.Holder<net.minecraft.world.entity.ai.attributes.Attribute> attr) {
    ...
    private static float applyReductionsIfSecure(LivingEntity target, float amount) {
        ...
        double red = readAttr(target, YizAttributes.DAMAGE_REDUCTION);
        if (red > 0) amount *= (float) (1.0 - Math.min(1.0, red / 100.0));
        double block = readAttr(target, YizAttributes.DAMAGE_BLOCK);
