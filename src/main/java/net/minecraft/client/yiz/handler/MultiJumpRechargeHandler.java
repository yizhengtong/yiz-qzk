...
/**
 * 多段跳落地充能 + 空中 cap — 服务端权威（仅玩家）。
 *
 * <p>等价 yizxian {@code ExtraJumpHandler}，三路径充能：
 * <ol>
 *   <li>{@link LivingFallEvent}：正常摔落落地（有 fallDistance）→ 充满</li>
 *   <li>{@code isFallFlying()} 下降沿：心之翅/鞘翅飞行结束 → 充满</li>
 *   <li>稳定地面 3 tick：{@code onGround && !flying && |vy|<0.1} 连续 3 tick → 充满
 *       （过滤空中 onGround 单 tick 误判）</li>
 * </ol>
...
public final class MultiJumpRechargeHandler {
    ...
    /** PlayerTickEvent.Post：飞行下降沿 + 稳定地面充能 + 空中 cap。由 tizMod 注册。 */
