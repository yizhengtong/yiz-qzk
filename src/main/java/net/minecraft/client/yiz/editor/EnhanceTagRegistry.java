...
public final class EnhanceTagRegistry {
    ...
    private static void addAttackModifier(ServerPlayer player, String key,
        ...
        if (inst == null) return;
        var rl = net.minecraft.resources.ResourceLocation.parse(key);
        inst.removeModifier(rl);
        inst.addTransientModifier(new net.minecraft.world.entity.ai.attributes.AttributeModifier(
            rl, value, net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADD_VALUE));
    ...
    private static void tickAttackModifier(ServerPlayer player, String key,
        ...
        if (n <= 0) {
            pd.remove(key + "_n");
            var inst = player.getAttribute(attr);
            if (inst != null) inst.removeModifier(net.minecraft.resources.ResourceLocation.parse(key));
        } else {
            pd.putInt(key + "_n", n);
        }
    }
    ...
    public static void tickLeixiaoshan(ServerPlayer player) {
        ...
        if (player.level().getGameTime() >= pd.getLong("yiz:leixiaoshan_t")) {
            ...
            if (stacks <= 0) {
                ...
                if (inst != null) inst.removeModifier(net.minecraft.resources.ResourceLocation.parse("yiz:leixiaoshan"));
                return;
            }
            pd.putInt("yiz:leixiaoshan_s", stacks);
            pd.putLong("yiz:leixiaoshan_t", player.level().getGameTime() + 5 * 20);
    ...
    private static void applyLeixiaoshan(ServerPlayer player) {
        ...
        inst.removeModifier(rl);
        inst.addTransientModifier(new net.minecraft.world.entity.ai.attributes.AttributeModifier(
            rl, cdr, net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADD_VALUE));
    }
