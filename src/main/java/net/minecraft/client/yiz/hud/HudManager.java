...
public final class HudManager {
    ...
    /** 默认位置按 960×540（1080p@GUI scale2）设计，当前分辨率不同时按比例换算。 */
    private static final int REF_W = 960;
    private static final int REF_H = 540;

    private static double[] guiRatio() {
        try {
            var w = Minecraft.getInstance().getWindow();
            return new double[] {
                w.getGuiScaledWidth()  > 0 ? (double) w.getGuiScaledWidth()  / REF_W : 1.0,
                w.getGuiScaledHeight() > 0 ? (double) w.getGuiScaledHeight() / REF_H : 1.0
            };
        } catch (Exception e) { return new double[]{1.0, 1.0}; }
    }
    ...
    public static int getX(HudElement e) {
        ...
        if (en != null) return en.x();
        double[] r = guiRatio();
        return (int) Math.round(e.getDefaultX() * r[0]);
    }

    public static int getY(HudElement e) {
        HudPositionConfig.Entry en = HudPositionConfig.get(e.getId());
        if (en != null) return en.y();
        double[] r = guiRatio();
        return (int) Math.round(e.getDefaultY() * r[1]);
    }
