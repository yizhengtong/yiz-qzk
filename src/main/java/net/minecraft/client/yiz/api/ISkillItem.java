...
public interface ISkillItem extends IEnhanceable {
    ...
    default int getCooldownTicks(ItemStack stack) {
        ...
        for (var e : mods.modifiers()) {
            if (e.attribute().is(net.minecraft.client.yiz.attribute.YizAttributes.COOLDOWN_VALUE))
                val += e.modifier().amount();
        }
    ...
    default void onActivate(int slot, Player player, ItemStack stack) {
        if (player instanceof net.minecraft.server.level.ServerPlayer sp) {
            net.minecraft.client.yiz.editor.EnhanceTagRegistry
                .executeActiveTags(sp, stack, slot, SkillTriggerType.ACTIVATE, null);
        }
    }
     ...
     */
    default void onTrigger(int slot, Player player, ItemStack stack,
                           net.minecraft.world.entity.LivingEntity target) {
        if (player instanceof net.minecraft.server.level.ServerPlayer sp) {
            net.minecraft.client.yiz.editor.EnhanceTagRegistry
                .executeActiveTags(sp, stack, slot, SkillTriggerType.TRIGGER, target);
        }
    }
     ...
     * 默认空实现。由 {@code SkillConfigMenu.removed} 经 {@link net.minecraft.client.yiz.api.EffectRegistry}
     * 间接驱动——技能类在 onCast 产生效果时向 EffectRegistry 登记 onUnequip 等价清理回调。</p>
     */
