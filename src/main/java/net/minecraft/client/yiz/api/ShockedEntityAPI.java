...
/**
 * 客户端感电状态机 — 维护"哪些实体正在感电"的持续状态（entityId → ShockedEntry）。
 *
 * <p>仿 {@code EntityLockAPI.CLIENT_CACHE}。服务端 {@code S2CShockFxPayload}（kind=0/BURST 次目标）
 * 调 {@link #putClient} 注册感电实体；{@code LivingEntityRendererSurfaceLightningMixin} 渲染时查
 * {@link #isShocked}/{@link #getFade} 决定是否叠加表面闪电 + 渐显渐隐。{@link #tick} 由
 * ClientTickEvent 每 tick 衰减，到期自动移除。</p>
 *
...
public final class ShockedEntityAPI {
    ...
    public static void putClient(int entityId, int durationTicks) {
        System.out.println("[ShockedEntityAPI] putClient id=" + entityId + " dur=" + durationTicks);  // 临时诊断
