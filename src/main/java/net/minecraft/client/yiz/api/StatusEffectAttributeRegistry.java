...
public final class StatusEffectAttributeRegistry {
    ...
    /** 攻方属性条目：Holder → 效果类型 */
    private static final Map<Holder<Attribute>, StatusEffectType> ATTACK_ENTRIES = new ConcurrentHashMap<>();

    /** 防方属性条目：Holder → 效果类型 */
    private static final Map<Holder<Attribute>, StatusEffectType> DEFENSE_ENTRIES = new ConcurrentHashMap<>();
     ...
     */
    public static void registerAttack(Holder<Attribute> holder, StatusEffectType type) {
     ...
     */
    public static void registerDefense(Holder<Attribute> holder, StatusEffectType type) {
    ...
    public static Map<StatusEffectType, Float> getAttackEffects(LivingEntity attacker) {
        ...
        for (var entry : ATTACK_ENTRIES.entrySet()) {
            var inst = attacker.getAttribute(entry.getKey());
    ...
    public static Map<StatusEffectType, Float> getDefenseEffects(LivingEntity defender) {
        ...
        for (var entry : DEFENSE_ENTRIES.entrySet()) {
            var inst = defender.getAttribute(entry.getKey());
