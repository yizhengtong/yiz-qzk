...
>
> 🟢 **2026-09-22 第六轮（铁斗士追击动画不播）**：根因＝客户端动画分支用了 `entity.getTarget()`，而**原版 `Mob.getTarget()` 不进同步通道**（`Mob` 只有 `DATA_MOB_FLAGS_ID` 一个 DataItem，`setTarget` 只写普通字段，`isAggressive()` 也读普通字段）⇒ 客户端恒 null ⇒ 模型里 `hasTarget` 恒假、永远播待机（攻击/技能动画正常，因为它们走 `broadcastEntityEvent`）。顺带修掉死分支：`walkState` 的 `moving && !chasing` 在"有目标"时恒假，WALK 从来没播过。修法＝服务端判定 `LOCOMOTION` 档位（0 静止/1 行走/2 追击，判"在动"用 `walkAnimation.isMoving()`——它按真实位移算）走 DataHolder 通道同步、客户端只做渲染分支。详见 `client-animation-server-sync`。
>
> 🟢 **2026-09-22 第七轮（怒翼：幻翼类一费飞行棋子）**：新增 `nuyi` 生物（`NuyiEntity`/`NuyiModel`/`NuyiRenderer` + 三张星级蛋），用户给模型+纹理，要求"直接复用幻翼 AI"。定策＝**继承 `YizxianMob`**（要全套防护，不是 `extends Phantom`）⇒ 幻翼飞行/AI 逐段搬运，四个硬点：①**重力在 `LivingEntity.travel` 里**，飞行必须整体覆写 travel 且**不能调 super**（基类的 super 会走重力路径直接坠地），速度全靠 MoveControl 写 `deltaMovement`（0.2 插值系数）；②目标扫描盒必须够高 `inflate(16,64,16)`——幻翼在目标上空 20~40 格盘旋，`NearestAttackableTargetGoal` 的等边盒永远扫不到人（这正是原版自写目标 goal 的原因），并照抄"按 Y 降序取最高"；③**碰撞伤害节流必须自己判**：基类 `doHurtTarget` 是破无敌帧写入，每 tick 无条件调＝80 点/秒，现行判 `target.invulnerableTime > 0` 跳过 ⇒ 每目标 ≤1 次/秒，伤害走 `ATTACK_DAMAGE`（模板 4，随难度/星级缩放）；④**Blockbench 导出件部件坐标是世界摆位**（y≈17~20）⇒ 渲染器**不要**叠加原版 `PhantomRenderer.scale` 的 `translate(0,1.3125,0.1875)`（那是给部件在 y≈0 的原版模型补管线偏移的），叠了会整体沉到碰撞箱下约 0.9 格；但 `setupRotations` 的 `mulPose(Axis.XP.rotationDegrees(getXRot()))` 要留（俯冲俯仰）。目标＝玩家 + `Enemy` 接口敌对生物（本模组棋子不实现 Enemy，天然不互锁）。数值口径（用户确认）：模板 12 血 / 4 攻，走难度缩放（困难 12/4、普通 9/3）× 星级 ×1/×1.5/×2.25。详见 `nuyi-phantom-family-piece`。
>
...
- [怒翼：幻翼 AI 搬到受保护棋子](nuyi-phantom-family-piece.md) — 加飞行类生物必读：重力在 `LivingEntity.travel` 里(覆写 travel 不能调 super)/目标盒要高 `inflate(16,64,16)`(等边盒扫不到盘旋中的目标)/碰撞伤害节流自己判无敌帧(基类 doHurtTarget 破无敌帧)/导出件坐标是世界摆位(别再叠加 PhantomRenderer 的位置补偿)；含一费数值口径与蛋图标生成法
...
- [客户端动画要服务端同步](client-animation-server-sync.md) — ⚠️原版 `Mob.getTarget()` **不进同步通道**（Mob 只有 `DATA_MOB_FLAGS_ID`，`setTarget` 只写普通字段，`isAggressive()` 也读普通字段）⇒ 客户端恒 null：铁斗士「追击移动不播动画」就是这个坑；规矩=客户端动画分支用的服务端状态一律自己开同步通道（DataHolder/姿态包），判"在动"用 `walkAnimation.isMoving()`（按真实位移算，不是 getDeltaMovement）；含铁斗士 LOCOMOTION 档位实现与排查清单
- [生物配置组件化](creature-component-system.md) — 对标 1.21 DataComponent 自建 ComponentType/ComponentMap/ComponentPatch；容器 + InstanceEffectState 重构 + 原型/数据包双轨 + 属性·战斗·形态组件 + 技能派发 + 诊断总开关均已落地；**2026-09-22 补：原型的 `effects` 之前是「配了不生效」（判定没接原型那一档），现判定顺序=实例补丁→原型→类级；铁斗士已加「免疫外部动量」`knockback_immunity`（代码轨原型 + JSON 双写）；⚠️同 id 数据包轨整体覆盖代码轨，改效果必须两处一起改**；仅普界者形态 phases 与存量全量迁移待做
