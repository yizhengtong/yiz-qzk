...
description: "辖界者真实血量外部哈希表存储 + 实体 override 血量方法 + 传导限伤（CONDUCTION_CAP/INTERVAL 属性）+ 外部模组防秒杀。给新 Boss 做受保护血量必读"
metadata:
  type: project
---

# 实体生命值保护补强（2026-08-07 落地 + 多轮重构，最终 = 外部表方案）

用户参考旧项目 `D:\ZM\yizgzq\jar\yiz-main` + 外部表方案（`D:\ZM\yizgzq\jar\外部表方案`，`ProtectedWeakHashMap` + 实体 override）思路，做「所有自研血量实体通用」的生命值保护。**终态：真实血量存外部哈希表，实体 override 血量方法完全接管**——外部模组（如外部模组）无法靠 setHealth(0)/巨伤/die() 秒杀。

## 血量外部存储（SecureHealthClosure，外部表 HealthManager 式）
...
外部模组（外部模组 `外部模组攻击实现`）攻击不走 vanilla hurt 链——`onLeftClickEntity` 里先 `target.hurt(MAX)` 再 `target.setHealth(0)` 再**自实现 die()（不调 target.die，直接 victim.dead=true + dropAllDeathLoot + setPose(DYING)）**。所以必须实体 override 全套：
...
- **外部模组连点**：每刀 hurt(真伤) + setHealth(0) 重定向 = 两次扣血，CD 让只吃第一下（真伤那次），连点 1 秒内后续全挡。
...
- **⚠️ 移除保护 isYizCaller 栈帧跳漏（2026-08-07 修）**：`EntityRemoveProtectionMixin.isYizCaller` 原从 i=3 遍历但没跳过 Mixin 注入帧 → 任何外部 remove 都被误判「本模组调用」放行（外部模组能移除辖界者）。已修：跳过 `Entity` 帧 + `EntityRemoveProtectionMixin` 帧，看第一个真实外部调用者。
...
- **外部模组疯狂变红**：`外部模组攻击实现.hurt` 绕过 hurt() 直接调 `victim.level().broadcastDamageEvent`（源码 161 行）→ 每次攻击都红、不经过传导 CD。**门控**：`ServerLevelDamageFlashProtectionMixin`（下游）拦 `ServerLevel.broadcastDamageEvent`，对 YizxianMob 仅当 `QuanshouzheEntity.isConductionHitFlash()`（ThreadLocal，hurt() 传导扣血时 set）才放行；外部直接调（标记未设）→ 拦截不红。已注册 yizxianmod.mixins.json。

## ⚠️ DataParameter 直写秒杀（外部模组 神明之剑，2026-08-07 防御）

- **外部模组 神明之剑**（`外部模组攻击实现` → `外部模组工具.easyAttack → killEntity → forceSetHealth`）：不调 `hurt()/setHealth()`，直接 `victim.entityData.set(DATA_HEALTH_ID, 0)` **绕过 setHealth override** → 客户端 getHealth 读 DataParameter 得 0 → 触发死亡/移除。Re-Avaritia 外部模组走 setHealth(0)（被 override 限伤挡），**外部模组 走 DataParameter 直写（override 挡不住）**——这是两者"一个挡得住一个挡不住"的根因。
- **外部表方案能挡**：`BossEntity.negateHealthDeltaSyncedData()` 每 tick 扫描 `SynchedEntityData` Float DataItem 写回 0。
- **辖界者防御（2026-08-07）**：
  1. `getHealth()` **客户端也读外部表**（S2C 同步真值，`isRegistered` 后才读表）→ 即使 DATA_HEALTH_ID 被写 0，客户端 getHealth 返回真值 → `isDeadOrDying` false → 不死。
  2. `customServerAiStep` 每 tick `EntityActuallyHurt.catchSetTrueHealth(this, 表血量)` 纠正 health 字段 + DATA_HEALTH_ID 回真值（外部表方案同思路）。
  3. `dropAllDeathLoot` override（表血量>0 拒）挡 its 反射掉落。

## ⚠️⚠️ removed/removalReason 字段直写 → 存档不保存（外部模组 终极移除，2026-08-08 防御）

- **现象**：游戏内辖界者没被移除，但**退出存档重进后消失**——存档 .mca 里根本没有辖界者实体。
- **根因（反编译 killEntity）**：外部模组 不只 setHealth/hurt，还用 Unsafe/反射**直接改 private 字段**：
  ```java
...
- **外部表方案能扛**：override `isRemoved`/`remove`/`setRemoved` 空实现 + 每 tick 纠正——实体永不真正移除。
- **辖界者三重防御（2026-08-08）**：
...
## ⚠️ MAX_HEALTH 属性保护（外部模组 等改最大生命值，2026-08-08 防御）

- **外部模组（外部模组）** `MaxHealthDrainConsequence.reduceMaxHealth(Player,double)`：`getAttribute(Attributes.MAX_HEALTH).addPermanentModifier(...)` 给 MAX_HEALTH 加 **permanent modifier** 降低最大生命值（虽签名收 Player，但任何模组都能对任意实体加 MAX_HEALTH modifier）。
...
前置库 build + 下游 runClient：玩家进世界→外部模组打辖界者→服务器正常退出（exit 0）。无 InjectionError/VerifyError（第三方 mod 的 agent retransform VerifyError 是已知旧行为）。`[QSZ]` 诊断日志验证：衰减→限伤→CD 节奏正确，血量按 25%/秒 下降非秒杀。

## 关联
- 辖界者 Boss 整体：[[warden-animation-reuse]]
- 属性鉴权设施：[[entity-attribute-gate]]
- 实体移除保护：[[entity-remove-protection]]
- 参考实现：`D:\ZM\yizgzq\jar\外部表方案`（HealthManager + BossEntity override）、旧项目 `D:\ZM\yizgzq\jar\yiz-main`
